...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Connect to JSON-RPC on <port> (default: %u or testnet: %u)"), 2051, 12051));
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("CBTC RPC client version") + " " + FormatFullVersion() + "\n" + PrivacyInfo();
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  cbtc-cli [options] <command> [params]  " + _("Send command to CBTC") + "\n" +
                  "  cbtc-cli [options] help                " + _("List commands") + "\n" +
                  "  cbtc-cli [options] help <command>      " + _("Get help for a command") + "\n";
