...
#endif // ENABLE_WALLET
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CBTCMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("CBTCMiner: ProcessNewBlock, block not accepted");
...
#endif
{
    LogPrintf("CBTCMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cbtc-miner");
    const CChainParams& chainparams = Params();
    ...
    try {
        while (true) {
            ...
            if (nHeight == -1) {
                LogPrintf("Error in CBTC Miner: chainActive.Height() returned -1\n");
                return;
            }
            ...
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in CBTCMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in CBTCMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            LogPrintf("Running CBTCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("CBTCMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("CBTCMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
