...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or cbtc.conf setting gen)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "cbtcd compiled without wallet and -mineraddress not set");
#endif
    }
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "CBTC is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "CBTC is downloading blocks...");
...
UniValue getblocksubsidy(const UniValue& params, bool fHelp)
{
    ...
    if (nextBlockHeight >= 295000 && nextBlockHeight <= 500000) {
        nCommunityFee = nReward * 0.90;
        nReward -= nCommunityFee;
    }
    else if (nextBlockHeight >= 700000 && nextBlockHeight <= 900000) {
        nCommunityFee = nReward * 0.90;
        nReward -= nCommunityFee;
    }
    else if (nextBlockHeight >= 170000 && nextBlockHeight <= 190000) {
        nCommunityFee = nReward * 0.90;
        nReward -= nCommunityFee;
    }
