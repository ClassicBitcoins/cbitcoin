...
PACKAGE_VERSION=$($SRC_PATH/src/cbtcd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/cbtcd $DEB_BIN
cp $SRC_PATH/src/cbtc-cli $DEB_BIN
...
# Copy manpages
cp $SRC_DOC/man/cbtcd.1 $DEB_MAN
cp $SRC_DOC/man/cbtc-cli.1 $DEB_MAN
cp $SRC_DOC/man/zcash-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/cbtcd.bash-completion $DEB_CMP/cbtcd
cp $SRC_PATH/contrib/cbtc-cli.bash-completion $DEB_CMP/cbtc-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/cbtcd.1
gzip --best -n $DEB_MAN/cbtc-cli.1
gzip --best -n $DEB_MAN/zcash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/cbtcd $DEB_BIN/cbtc-cli
