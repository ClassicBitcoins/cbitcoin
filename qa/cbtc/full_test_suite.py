...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/cbtc/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/cbtc/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if cbtc is an ELF binary, they all are
    with open(repofile('src/cbtcd'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/cbtcd')
    ret &= test_rpath_runpath('src/cbtc-cli')
    ret &= test_rpath_runpath('src/cbtc-gtest')
    ret &= test_rpath_runpath('src/cbtc-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/cbtcd')
    ret &= test_fortify_source('src/cbtc-cli')
    ret &= test_fortify_source('src/cbtc-gtest')
    ret &= test_fortify_source('src/cbtc-tx')
