...
Backing up your CBTC private keys is the best way to be proactive about preventing loss of access to your ZEC.
...
These instructions are specific for the officially supported CBTC Linux client. For backing up with third-party wallets, please consult with user guides or support channels provided for those services.
...
For all methods, you will need to include an export directory setting in your config file (`cbtc.conf` located in the data directory which is `~/.cbtc/` unless it's been overridden with `datadir=` setting):
...
Note that cbtcd will need to be stopped and restarted for edits in the config file to take effect. 

### Using `backupwallet`

To create a backup of your wallet, use:

`cbtc-cli backupwallet <nameofbackup>`.
...
If you generate a new CBTC address, it will not be reflected in the backup file.
...
`cbtc-cli z_exportwallet <nameofbackup>`
...
`cbtc-cli z_importwallet </path/to/exportdir/nameofbackup>`
...
`cbtc-cli z_exportkey <z-address>`
...
`cbtc-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`cbtc-cli z_importkey <z-priv-key>`
